...
public class JoinMRJob extends Configured implements Tool {
    ...
    public int run(String[] allArgs) throws Exception {
        ...
        job.setPartitionerClass(CarrierCodeBasedPartitioner.class);// 这个是用来设置key的切分，重写partition
        job.setSortComparatorClass(CarrierSortComparator.class);// 这个是设置key的比较器，设置聚合的key的一个二次排序方法
        job.setGroupingComparatorClass(CarrierGroupComparator.class); // 这里设置group，就是按key的哪一个参数进行聚合
